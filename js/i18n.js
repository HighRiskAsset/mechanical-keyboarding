  ...
  const STRINGS = {
    en: {
      ...
      welcomeRules: [
        ...
        '<b>Hold Space</b> wherever you stand to open that place\'s menu — build on a surveyed plot or ore vein, upgrade a mine to new keys, give a machine a different recipe, automate it, feed or empty it, take it down again. Arrows choose a row, a tap of Space confirms it, Escape closes. A caption under the map says what the chosen row does.',
      ...
      capBuild: (p) => `Build ${/^[aeiou]/i.test(p.kind) ? 'an' : 'a'} ${p.kind} here`,
      ...
      sideNames: { e: 'the right', s: 'the front', w: 'the left' },
      capTurn: (p) => `Turn it — its outlets move to ${p.side}, its inlets to the other two sides; its belts are re-laid to follow`,
      capTurnLost: (p) => `Turned — ${p.n} ${p.n === 1 ? 'run had' : 'runs had'} nowhere left to go and came up`,
      capUnbelt: (p) => `Take up the run from the ${p.from} to the ${p.to}`,
      capOnBelt: 'On a belt — hold Space to open it',
      capOnCrossing: 'On a crossing — hold Space to open it',
      capRemove: (p) => `Take the ${p.name} down — its price and everything inside come back to your bag; its belts come up with it`,
    ...
    ru: {
      ...
      welcomeRules: [
        ...
        '<b>Зажмите пробел</b> там, где стоите, — откроется меню этого места: построить машину на площадке или жиле, открыть руднику новые клавиши, задать машине другой рецепт, автоматизировать её, загрузить или опустошить, разобрать обратно. Стрелки выбирают строку, короткое нажатие пробела подтверждает, Escape закрывает. Подпись под картой говорит, что делает выбранная строка.',
      ...
      sideNames: { e: 'вправо', s: 'вперёд', w: 'влево' },
      capTurn: (p) => `Повернуть — выходы смотрят ${p.side}, входы уходят на две другие стороны; ленты перекладываются следом`,
      capTurnLost: (p) => `Повернули — лент без пути: ${p.n}, они сняты`,
      capUnbelt: (p) => `Снять ленту: ${p.from} → ${p.to}`,
      capOnBelt: 'Вы на ленте — зажмите пробел, чтобы открыть её',
      capOnCrossing: 'Вы на пересечении — зажмите пробел, чтобы открыть его',
      capRemove: (p) => `Разобрать: ${p.name} — цена и всё, что внутри, вернутся в сумку; ленты снимутся вместе с ней`,
