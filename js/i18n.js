  ...
  const STRINGS = {
    en: {
      ...
      welcomeRules: [
        ...
        '<b>Hold Space</b> at a machine to open its menu — upgrade a mine to new keys, give a machine a different recipe, automate it, feed or empty it, turn it, take it down again. Arrows choose a row, a tap of Space confirms it, Escape closes. A caption under the map says what the chosen row does.',
      ...
      capPlaceVein: 'A mine stands on a free vein — walk its ghost onto one, or hold Space to put it away',
      capPlacePoor: (p) => `${p.name} — not affordable yet · hold Space to put it away`,
      capTurnBlocked: 'No room to turn it — another machine stands where its body would swing',
      ...
      // the sides of a machine, as the player looks at the field
      sideNames: { e: 'the right', s: 'the front', w: 'the left', n: 'the back' },
      capTurn: (p) => `Turn it a quarter clockwise — body, doors and ports together, its outlets to ${p.side}; its belts are re-laid to follow`,
      capTurnLost: (p) => `Turned — ${p.n} ${p.n === 1 ? 'run had' : 'runs had'} nowhere left to go and came up; what ${p.n === 1 ? 'it was' : 'they were'} carrying is on the ground`,
    ...
    ru: {
      ...
      welcomeRules: [
        ...
        '<b>Зажмите пробел у машины</b> — откроется её меню: открыть руднику новые клавиши, задать другой рецепт, автоматизировать, загрузить или опустошить, повернуть, разобрать обратно. Стрелки выбирают строку, короткое нажатие пробела подтверждает, Escape закрывает. Подпись под картой говорит, что делает выбранная строка.',
      ...
      capPlaceVein: 'Рудник встаёт на свободную жилу — доведите призрак до неё или зажмите пробел, чтобы убрать',
      capPlacePoor: (p) => `${p.name} — пока не по карману · зажмите пробел, чтобы убрать`,
      capTurnBlocked: 'Не повернуть — там, куда легло бы тело, стоит другая машина',
      ...
      // стороны машины — как игрок смотрит на поле
      sideNames: { e: 'вправо', s: 'вперёд', w: 'влево', n: 'назад' },
      capTurn: (p) => `Повернуть на четверть по часовой — корпус, дверцы и порты вместе, выходы ${p.side}; ленты перекладываются следом`,
      capTurnLost: (p) => `Повернули — лент без пути: ${p.n}, они сняты; груз с них лежит на земле`,
